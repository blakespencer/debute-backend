...
import { swapRoutes, SwapService } from "./modules/swap";
...
app.get('/debug/test-unhandled-rejection', async (req, res) => {
  ...
  logger.info('🧪 Triggering unhandled promise rejection for testing', { module: 'debug', operation: 'test-unhandled-rejection' });

  // Create a promise that rejects without being caught
  Promise.reject(new Error('Test unhandled promise rejection from /debug/test-unhandled-rejection endpoint'));

  res.json({ message: 'Unhandled rejection triggered - check server logs for enhanced error details' });
});

app.get('/debug/test-uncaught-exception', async (req, res) => {
  // This will trigger an uncaught exception
  logger.info('🧪 Triggering uncaught exception for testing', { module: 'debug', operation: 'test-uncaught-exception' });
  ...
  setTimeout(() => {
    throw new Error('Test uncaught exception from /debug/test-uncaught-exception endpoint');
  }, 100);
});
...
app.use(
  (
    ...
    logger.errorWithUnknown("Unexpected error in Express middleware", error, { module: 'express', operation: 'error-handler' });
    res.status(500).json({
      success: false,
